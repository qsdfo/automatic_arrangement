...
% Our objective
The objective of our work is to produce a system able to automatically perform the projective orchestration of a piano score. To our best knowledge, this problem has never been tackled before. 
% How = Deep learning
Practically, our system will rely on statistical inference and more precisely on the most recent advent in machine learning. We will focus on a set of model called conditional Restricted Boltzmann Machine (cRBM).
% Explain mapping piano -> orchestra
The system we try to build take in input a piano score and produce an orchestra score and is hence set in a purely symbolic world (i.e. only music scores and no audio signal).
The following question arise : what are our motivations for using deep learning ? Isn't it pretentious to try to build an automatic system for orchestration even though our knowledge of its inner mechanism is very limited ? How could we obtain interesting results by considering a purely symbolic information if, as we stated it before, notation are not sufficient to express the complexity showed at the signal level ?
...
% Parler de l'échange on gérnère <-> on apprend (nous humain) des règles inférées par la machine
If trying to build an automatic system for orchestration may seem to ambitious, we actually believe that this  is rather the starting point of the knowledge quarry task we want to accomplish. By trying to mimic famous composer, we might discover the higher level knowledge we are trying to discover.




Hence, The training phase consists in trying to discover the correlation between the original piano score and its orchestration . Statistical inference would allow us to extract rules from the vast knowledge embodied in the many orchestration proposed by famous composers over the years. 

% Why only symbolic
In the light of what has been said about the multi-modal aspect of orchestration, it can seems absurd to 
% Why DL : requirement = observing the learnt knowledge
Humans have difficulties fail at explaining and formalizing the complex and high level knowledge they invoke when writing an orchestration, we could observe from such a system the rules and structures it has discovered and then use it to increase our understanding of orchestration.
Our motivations are then double : being able to automatically produce orchestration, but above all discovering and formalizing the knowledge implied by orchestration. This article constitutes a first step toward this aspiration.

% Motivations
			% Pourquoi ça peut marcher (emboddied knowledge)...
