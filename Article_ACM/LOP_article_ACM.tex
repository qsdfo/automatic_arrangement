...
\textit{Musical orchestration} is the subtle art of writing musical pieces for the orchestra, by combining the different instruments in order to create a particular sound. Given the number of different instruments in a symphonic orchestra and their respective range of notes, one can foresee the huge number of combination to consider in an orchestration problem.
Beside this combinatorial problem, the resulting sound of an even simple instrumental mixture is particularly difficult to predict, especially its emerging timbre. 
Several attempts to mathematically characterize the timbre as a set of spectro-temporal descriptors have outputted the complex non linear behaviours involved.
Those difficulties have probably been an obstacle to the construction of a theory of orchestration and it remains a mainly empirical discipline, taught through the observation of already existing orchestrations (\cite{piston-orch}).
A classic exercise, which is actually a way to produce orchestral music, consists in two steps.
An harmonic, melodic, and rhythmic structure is first written in the form of a piano score (or any polyphonic instrument) the timbre dimension being at this point discarded. This structure is then projected on an orchestra, by spreading the harmonic, melodic and rhythmic structure over the different instruments. This second step, the mapping from a piano score to an orchestral score, is often referred to as \textit{projective orchestration}.

We believe that embodied in the many projective orchestrations already performed by famous composers lie rules and  regularities that human being are unable to extract due to the dimensionality of the space considered. 
Considering that all the information about instrumental mixtures is contained at a symbolic level is a strong assumption, but reasonable since those examples have been composed based on a certain knowledge.

Our objective is to build a system able to automatically perform projective orchestration. 
In a first step, it tries to model the probabilistic distribution of the orchestral score conditionally on the piano score through statistical inference methods. This

Lie at symbolic level
By using automatic inference, we 

%% Projective and injective. SOA automatic injective orchestration
The wide definition we gave of orchestration actually covers many different sub-problems from which arise two different paradigms for writing orchestral music. Among those, we can distinguish \textit{injective} and \textit{projective} orchestration \cite{eslingthesis}. In the \textit{injective} paradigm, the fundamental problem is to find a mixture of instrument which sounds as close as possible, in term of timbre, of a particular target sound (for instance a crying baby). A formal definition can be given through the definition of a timbral target instead of the vague concept of target sound, which is the evolution in time of a set of spectral descriptors. In this framework, rhythmic, harmonic and melodic structures emerge as the background of the timbral landscape depicted.

\textit{Projective} orchestration can be seen as the projection of a piano score on an orchestra (see figure \ref{fig:orch}).
In this paradigm, the composition process rely on an already existing harmonic, rhythmic and melodic structure and propose a timbral evolution over it. A limited yet helpful analogy would be consider it as colouring a black and white picture.
The classic and romantic repertoires contain a huge amount of example (e.g. the piano reduction by Liszt of the symphonies of Beethoven, \textit{les tableaux d'une exposition}, a piano piece from Modest Moussorgsky orchestrated by Ravel).

% Injective : Orchids
A computer-assisted tool for orchestration have already been developed in the injective framework. Orchids \cite{esling2010dynamic} is a system based on a genetic algorithm which try to find a mixture of instrument as close as possible of a given timbre target. Several measures based on spectro-temporal descriptors of the instruments are used to build a multi-objective criterion.

% Our objective
The objective of our work is to produce a system able to automatically perform the projective orchestration of a piano score. Its input is then a piano score and its output an orchestra score. To our best knowledge, this problem has never been tackled before. 
% How = Deep learning
Our system will rely on statistical inference in order to extract the correlations between an original piano scores and its existing orchestrations by famous composers.
More precisely, we will use the most recent advent in machine learning and a set of model called conditional Restricted Boltzmann Machine (\textit{cRBM}).
