...
\begin{abstract}
\noindent This article introduces the \textbf{POD} dataset, a freely-available collection of midi scores 
...
% C'est quoi aligner ? C'est aligner les séquences d'accords formées par le pianoroll
More precisely, we consider the piano-roll representations (\prettyref{fig:pianoroll}) of scores. Hence, a score can be represented as a sequence of vectors. Then, if we manage to define a distance between chords, the problem of aligning two scores can be casted as a classic sequence alignment problem.
