...
%The \textit{cRBM} can efficiently model the influence of each note in the context vector $\left[P(t), O(t-N), ..., O(t-1)\right]$ over the each note in the visible vector $O(t)$. 
%However, considering the influence of pairs of units $(x,z)$ from respectively $P(t)$ and $\left[ O(t-N), ..., O(t-1)\right]$ over the visible units $O(t)$ could provide a more powerful model. This is done in the Gated conditional RBM, by introducing latent variables $\bm{z}=\{z_1,.., z_l, ..,z_{n_z}\}$, and tying together variables $\bm{v}$ (respectively $\bm{h}$), $\bm{x}$ and $\bm{z}$ through a three dimensional tensor $\bm{B}^v = (B_{ikl})^v$ (respectively $\bm{B}^h$).
%The number of parameters rapidly explodes with the dimension of the visible, context and latent vectors and the model consequently become impossible to train. A solution consists in factorizing the three dimensional tensors through a set of factor units $\bm{f} = (f_1,.., f_f,.., f_{n_f})$ as $\bm{B}^v = (\bm{B}_{if}^v . \bm{B}_{kf}^v .  \bm{B}_{lf}^v)$.
