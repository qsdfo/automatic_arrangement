...
class RBM_temporal(object):
    ...
    def prediction_error(self, k=20):
        # Generate input knowing the past by sampling
        # Run a Gibbs sampling chain with clamped past units
        visible_chain, updates = theano.scan(lambda v, p: gibbs_step(v, p)[0],
                                             outputs_info=self.input,
                                             non_sequence=self.past,
                                             n_steps=k)
        v_pred = visible_chain[-1]
        error = f1_measurement(self.input, v_pred)
        # error is a vector of size minibatch_size
        return error
