#!/usr/bin/env python
# -*- coding: utf-8 -*-
from Data_processing.MXML_parser.mxml_parser import build_db
from Visualization.Pianoroll.pianoroll_to_csv import pianoroll_to_csv

if __name__ == '__main__':
    database_path = '../Database/LOP_db_small/'
    quantization = 4
    instru_dict_path = '../Database/LOP_db_small/instrument_dico.json'
    pickled_path = '../Data/data.p'
    CSV_path = 'Visualization/Pianoroll/CSV_pianoroll/'

    # Build the pickle database (data.p file)
    build_db(database_path, quantization, instru_dict_path, pickled_path)
    # Build the CSV directory for visualization purposes
    # (abolutely not required, but one might want to check the value loaded from the .p file)
    pianoroll_to_csv(pickled_path, CSV_path)
