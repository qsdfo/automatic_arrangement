...
# MODEL
from Models.LSTM_plugged_base import LSTM_plugged_base as Model
# DATABASE
...
############################################################
model_space = Model.get_hp_space()
optim_space = {}
...
if DEFINED_CONFIG:
    for config_id, model_space in configs.iteritems():
        ...
        space = {'model': model_space, 'optim': optim_space, 'parameters': parameters}
        pkl.dump(space, open(config_folder + '/config.pkl', 'wb'))
        
        # Training
        run_wrapper(space, config_folder)
...
def run_wrapper(space, config_folder):

    parameters = space['parameters']
    model_params = space['model_space']
    optim_space = space['optim_space']
    ...
    piano_train, orchestra_train, train_index \
        = load_data_train(parameters['data_folder'],
                          None, None,
    ...
    piano_valid, orchestra_valid, valid_index \
        = load_data_valid(parameters['data_folder'],
                          None, None,
                          model_params['temporal_order'],
                          parameters['batch_size'],
                          skip_sample=parameters['skip_sample'],
    ...
    piano_test, orchestra_test, _, _ \
        = load_data_test(parameters['data_folder'],
                         None, None,
                         model_params['temporal_order'],
                         parameters['batch_size'],
                         skip_sample=parameters['skip_sample'],
    ...
    dimensions = {'batch_size': parameters['batch_size'],
