...
def train(model,
    ...
    loss = tf.reduce_mean(keras.losses.binary_crossentropy(labels, preds))
    ...
    while (not OVERFITTING and not TIME_LIMIT
        ...
        for batch_index in train_index:
        	# Build batch
        	piano_t = piano_train[index]
        	orchestra_past = build_sequence(orchestra_train, index-1, model.batch_size, model.temporal_order-1, model.orchestra_dim)
        	orchestra_t = orchestra[index]
	        
	        # Train step
	        train_step.run(feed_dict={model.piano_t: piano_t,
            						  model.orchestra_past: orchestra_past,
            						  model.orchestra_t: orchestra_t,
            						  K.learning_phase(): 1})
