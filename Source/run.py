...
def run_wrapper(params):
    ...
    # Paths
    ############################################################
    ALREADY_USED_ID = True
    while ALREADY_USED_ID:
        ID = str(random.randint(1,1000000))
        CONFIG_FOLDER = script_param['result_folder'] + '/' + ID
        if not os.path.isdir(CONFIG_FOLDER):
            os.mkdir(CONFIG_FOLDER)
            ALREADY_USED_ID = False
    ...
    logger_run.info((u'# Config ID :  {}'.format(ID)).encode('utf8'))
    ...
    ############################################################
    # Pickle (save) the model and optim
    # (just to keep track of the full config used
    # to obtain this particular result)
    ############################################################
    save_model_file = CONFIG_FOLDER + '/model.pkl'
    with open(save_model_file, 'wb') as f:
        pkl.dump(model, f, protocol=pkl.HIGHEST_PROTOCOL)

    save_optim_file = CONFIG_FOLDER + '/optim.pkl'
    with open(save_optim_file, 'wb') as f:
        pkl.dump(optimizer, f, protocol=pkl.HIGHEST_PROTOCOL)

    ############################################################
    # Return an hyperopt dict :
